...
/**
 * Automated Auth Performance Test
...
async function testBackendHealth() {
  ...
  try {
    ...
    if (error.code === 'ECONNABORTED') {
      ...
      logError('Connection refused - backend is not running');
    }
  ...
  try {
    ...
    if (response.status === 201) {
      ...
      if (response.data.token) {
        ...
        timings.tokenGeneration = 'included';
      }
    ...
    if (error.response) {
      logError(`Registration failed: ${error.response.data.message || error.response.statusText}`);
      logInfo(`Status code: ${error.response.status}`);
      
      if (error.response.status === 400) {
        logWarning('Validation error - user might already exist');
      } else if (error.response.status === 500) {
        logError('Server error - check backend logs');
      }
    } else if (error.code === 'ECONNABORTED') {
      logError('Request timeout - registration took >30s');
    } else {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      if (response.data.userId || response.data.id) {
        logInfo(`User ID: ${response.data.userId || response.data.id}`);
      }
    ...
    if (error.response) {
      logError(`Login failed: ${error.response.data.message || error.response.statusText}`);
      logInfo(`Status code: ${error.response.status}`);
      
      if (error.response.status === 401) {
        logError('Invalid credentials - password mismatch or user not found');
      ...
      logError('Request timeout - login took >30s');
    } else {
      logError(`Network error: ${error.message}`);
    }
...
async function testTokenValidation(token) {
  logStep(4, 'Testing Token Validation');
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    logTiming('Token validation time', duration);
...
async function generatePerformanceReport(results) {
  logStep(5, 'Performance Analysis Report');
  ...
  if (results.validation) {
    logTiming('Token Validation', results.validation.duration);
  }
...
async function runTests() {
  ...
  try {
    ...
    // Test 4: Token Validation (if login succeeded)
    if (results.login.success && results.login.token) {
      await new Promise(resolve => setTimeout(resolve, 500));
      results.validation = await testTokenValidation(results.login.token);
    }
