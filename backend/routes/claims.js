  ...
  try {
    ...
    if (item.isImageHidden && item.verificationQuestions && item.verificationQuestions.length > 0) {
      ...
      if (answers.length !== item.verificationQuestions.length) {
        return res.status(400).json({ error: 'You must answer all verification questions to claim this protected item.' });
      }
      item.verificationQuestions.forEach((q, idx) => {
...
// Get claims for a specific item (only accessible by item owner)
router.get('/item/:itemId', auth, async (req, res) => {
  try {
    const item = await Item.findById(req.params.itemId);
    if (!item) return res.status(404).json({ error: 'Item not found' });

    // Only the person who reported the item can see its claims
